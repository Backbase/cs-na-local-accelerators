...
import { RestrictedDatesHttpService, RestrictedDates } from '@backbase/common-data-transfers-dates-http-ang';
...
export class RestrictedDatesService {
  ...
  isDateRestricted$: Observable<(date: string) => boolean> = this.restrictedDates$.pipe(
    map((dates) => {
  ...
  restrictedDateValidator(): AsyncValidatorFn {
